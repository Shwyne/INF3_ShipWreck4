...
void ShipWreck::getData_(string& input){
  ...
  if(delimiter!= ';'){
    std::cerr << "Unvalid format: Expecting x;y as string" << std::endl;
  }
  else{
    status = "";
  }
...
string ShipWreck::sendData_(){
  return status;
}

void ShipWreck::calcShot(){
  N_++;
  res = w.shoot(X_, Y_);
  ...
  status = "GAME OVER";
    ...
    status = statusToString(res);
  }
}

void ShipWreck::output(){
  cout << std::endl << "-----------------------------------------------------" << std::endl;
  cout << N_ << ".Versuch:" << std::endl << std::endl;
  w.printBoard();
}

World::World(int maxX, int maxY, int nmbFiver, int nmbFourer, int nmbThreer,
             int nmbTwoer) {
  if (maxX < 10) {
    maxX_ = 10;
  } else {
...
bool World::checkNeighborhood(int x, int y) {
  if (((x + 1) <= maxX_) && ((x - 1) > 0)) {
    if (coordAlreadyUsed(x - 1, y) == BLOCK_USED) {
      return true;
    };
    if (coordAlreadyUsed(x + 1, y) == BLOCK_USED) {
      return true;
  ...
  if (((y + 1) <= maxY_) && ((y - 1) > 0)) {
    if (coordAlreadyUsed(x, y - 1) == BLOCK_USED)
      return true;
    if (coordAlreadyUsed(x, y + 1) == BLOCK_USED)
      return true;
  }
  ...
  if ((((x + 1) <= maxX_) && ((x - 1) > 0)) &&
    ...
    if (coordAlreadyUsed(x, y + 1) == BLOCK_USED)
      return true;
    if (coordAlreadyUsed(x + 1, y - 1) == BLOCK_USED)
      return true;
    if (coordAlreadyUsed(x + 1, y + 1) == BLOCK_USED)
      return true;
    if (coordAlreadyUsed(x - 1, y - 1) == BLOCK_USED)
...
bool World::placeSingleShip(int idxShip) {
  ...
  do { // get rand start coordinates
    nmbTries++;
    if (nmbTries > maxTries)
      return false; // give up no place for ship found
...
void Ship::print() {
  cout << "type: " << shipname_ << " sate: " << state_
       << " nmbBlocks: " << nmbBlocks_ << endl;
...
void World::print() {
  cout << "maxX: " << maxX_ << " maxY: " << maxY_ << " nmbShips " << nmbShips_
       << endl;
...
void World::printBoard() {
  ...
  string statusToString(ShootResult status){
    static const std::map<ShootResult, std::string> statusMap = {
        {ShootResult::WATER, "You've shot the water! Yikes!"},
        {ShootResult::SHIP_HIT, "Boom! You've hit a ship!"},
        {ShootResult::SHIP_DESTROYED, "SOS, Ships sinking!"},
        {ShootResult::ALL_SHIPS_DESTROYED, "Congratulations! You've cleared the board!"},
