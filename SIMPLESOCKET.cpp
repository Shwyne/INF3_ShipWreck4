...
TCPserver::TCPserver(int port, int maxDataSizeRecv) {
  ...
  ipOfServer_.sin_port =
      htons(port); // this is the port number of running server

  if(bind(clintListn_, (struct sockaddr *)&ipOfServer_, sizeof(ipOfServer_)) < 0){
    perror("bind failed");
    exit(1);
  }
}
...
TCPserver::~TCPserver() { delete[] dataRecv_; }
